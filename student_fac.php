<?php
	...
	include('header.php');
	$handle = fopen("entry_nos.txt", "r");
	$initial = microtime(true);
	if ($handle) {
		while ($line = fgets($handle)) {
			$line = trim($line);
                        //echo $line;
			$content = file_get_contents("http://internal.iitd.ernet.in/search/ldap1.php?uid=".$line);
                        //echo $content;
			$GLOBALS['a']=array();
			$name = "";
			$studentid = "";
			...
			pg_query("insert into student values ('".$GLOBALS['a'][10]."','".$GLOBALS['a'][10]."','".$GLOBALS['a'][11]."','".pg_escape_string($GLOBALS['a'][13])."',".$GLOBALS['a'][12].",'".pg_escape_string($GLOBALS['a'][8])."','".$GLOBALS['a'][0]."','".$GLOBALS['a'][1]."','".$GLOBALS['a'][2]."','".$GLOBALS['a'][9]."','".$GLOBALS['a'][3]."','".$GLOBALS['a'][4]."','".$GLOBALS['a'][5]."','".$GLOBALS['a'][6]."','".pg_escape_string($GLOBALS['a'][7])."');");
